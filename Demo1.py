...
secno = 'SH688235'
df=get_price(secno,frequency='1d',count=5)  #分钟线行情，只支持从当前时间往前推，可用'1m','5m','15m','30m','60m'
df.insert(0, 'secno', secno)
connect = create_engine('mysql+mysql://{'pythonpractiseadmin'}:{'!q2w3e4r'}@{'localhost'}:{3306}/{'PythonPractise'}?charset=utf8')
df.to_sql('shareprice',con=connect,if_exists='replace',index=False)
print('百济神州日行情\n',df)